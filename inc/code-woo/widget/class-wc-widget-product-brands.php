...
class WC_Widget_Product_Brand extends WC_Widget {
	...
	public function __construct()
	{
		...
		$this->settings           = array(
			...
			'orderby'            => array(
				...
				'options' => array(
					'order' => __( 'brands order', 'woocommerce' ),
	...
	public function widget($args, $instance)
	{
		...
		if( $dropdown ) {
				...
				apply_filters(
					...
					wp_parse_args(
						...
						array(
							...
							'taxonomy'           => 'product_brand',
							'name'               => 'product_brand',
							'class'              => 'dropdown_product_cat dropdown_product_brand',
						)
					)
				)
			);
				...
				if ( jQuery().selectWoo ) {
					var wc_product_brand_select = function() {
						jQuery( '.dropdown_product_brand' ).selectWoo( {
							placeholder: '" . esc_js( __( 'Select a brands', 'woocommerce' ) ) . "',
			...
			$list_args['walker']                     = new WC_Product_Brand_List_Walker();
			$list_args['title_li']                   = '';
