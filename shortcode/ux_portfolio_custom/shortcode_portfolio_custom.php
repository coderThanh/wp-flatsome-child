...
function pt_portfolio_shortcode($atts, $content = null, $tag = '')
{
	...
	if( !$style )
		$style = flatsome_option( 'portfolio_style' );
	...
	if( $filter && $filter != 'disabled' && empty( $cat ) && $type !== 'grid' && $type !== 'slider' && $type !== 'full-slider' ) {
		...
		<div class="container mb-half">
			<ul
				class="nav nav-<?php echo $filter; ?> nav-<?php echo $filter_align; ?> nav-<?php echo $filter_nav; ?> nav-uppercase filter-nav">
				...
				foreach( $tax_terms as $key => $value ) {
					?>
					<li><a href="#"
							data-filter="[data-terms*='<?php echo "&quot;" . $value->name . "&quot;"; ?>']"><?php echo $value->name; ?></a>
					</li><?php
				}
				?>
			</ul>
	...
	if( $wp_query->have_posts() ) :
		...
		while( $wp_query->have_posts() ) :
			...
			$terms_string = strip_tags( get_the_term_list( get_the_ID(), 'featured_item_category', "", "," ) );

			$terms_list = explode( ',', $terms_string ) ?? [];

			$terms_html = array_map( function ($name) {
				return '<span>' . $name . '</span>';
			}, $terms_list );

            $terms_html = implode( '', $terms_html );
			?>
            
			<div class="<?php echo implode( ' ', $classes_col ); ?>"
				data-terms="<?php echo strip_tags( get_the_term_list( get_the_ID(), 'featured_item_category', "[&quot;", "&quot;,&quot;", "&quot;]" ) ); ?>"
				<?php echo $animate; ?>>
						...
						<div class="<?php echo implode( ' ', $classes_box ); ?>">
                            ...
                            <?php echo $terms_html;?>
								...
								<div class="box-text-inner">
									...
									<div class="portfolio-box-excerpt">
										<?php echo esc_attr( the_excerpt() ); ?>
									</div>
									<div class="portfolio-box-read-more"><?php echo esc_attr( __( 'Learn more' ) ); ?></div>
...
add_shortcode( "ux_portfolio_custom", "pt_portfolio_shortcode" );
