...
$icon_custom = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" class="icon" color="currentColor" fill="currentColor">
...
<?php if( get_theme_mod( 'header_search_style', 'dropdown' ) !== 'lightbox' ) { ?>
	<li class="header-search header-search-dropdown has-icon has-dropdown menu-item-has-children">
		<?php if( $icon_style ) { ?>
			...
			<a href="#" aria-label="<?php echo __( 'Search', 'woocommerce' ); ?>"
...
<?php } else if( get_theme_mod( 'header_search_style' ) == 'lightbox' ) { ?>
		...
		<?php if( $icon_style ) { ?>
				<div class="header-button"><?php } ?>
				<a href="#search-lightbox" aria-label="<?php echo __( 'Search', 'woocommerce' ); ?>"
